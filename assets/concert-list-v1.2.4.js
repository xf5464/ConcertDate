(() => {
  ...
  function currentFilteredConcerts() {
    ...
    return data.concerts
      ...
      .slice()
      .sort((a, b) => String(a.date).localeCompare(String(b.date)) || String(a.startTime || '').localeCompare(String(b.startTime || '')) || String(a.theater).localeCompare(String(b.theater)) || String(a.title).localeCompare(String(b.title)));
  }

  function formatDateMeta(item) {
    ...
    const start = item.startTime ? `开场 ${item.startTime}` : '开场时间未知';
    const duration = item.durationText || (Number.isFinite(item.durationMinutes) ? `${item.durationMinutes}分钟` : '时长未知');
    return `${year}年${month}月${day}日 · ${weekday} · ${start} · ${duration}`;
  }
      ...
      list.innerHTML = items.map(item => {
        ...
        return `
          <article class="concert-list-item">
            <div class="concert-list-meta">${formatDateMeta(item)}</div>
