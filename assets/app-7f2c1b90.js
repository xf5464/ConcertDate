...
function attachListeners() {
  ...
  weekdayGroup.addEventListener('click', event => {
    const button = event.target.closest('#weekdayDropdownButton');
    if (!button) return;
  ...
  document.addEventListener('click', event => {
    if (weekdayGroup.contains(event.target)) return;
    const panel = document.getElementById('weekdayDropdownPanel');
...
function renderMonth(year, month, eventsByDate) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const eventHtml = hasConcert ? '<div class="event-list"><span class="concert-marker" aria-hidden="true"></span></div>' : '';
