(() => {
  ...
  async function refreshFromRemoteIfNeeded() {
    try {
      const versionResponse = await nativeFetch(`${VERSION_URL}?t=${Date.now()}`, { cache: 'no-store' });
      ...
      const freshResponse = await nativeFetch(`${DATA_URL}?v=${encodeURIComponent(remoteVersion)}`, { cache: 'no-store' });
      if (!freshResponse.ok) return;
      const freshData = await freshResponse.json();

      if ('caches' in window) {
        const cache = await caches.open(DATA_CACHE);
        await cache.put(DATA_URL, new Response(JSON.stringify(freshData), {
          headers: { 'Content-Type': 'application/json; charset=utf-8' }
        }));
      }
      if (typeof window.refreshConcertData === 'function') window.refreshConcertData(freshData);
    } catch (error) {
      console.warn('Concert data version check failed', error);
    }
  }

  window.addEventListener('DOMContentLoaded', () => setTimeout(refreshFromRemoteIfNeeded, 0));
})();
